...
class SpanLabeledTextDataset(Dataset):
    def __init__(
        ...
        (
            self.list_of_tokens,
            self.list_of_token_ids,
            self.list_of_labels,
            self.list_of_label_ids,
            self.list_of_segment_ids,
            self.list_of_token_start_map,
        ) = ([], [], [], [], [], [])
    ...
    def _convert_to_features(self, words, labels, label_map, list_token_start_map):
        ...
        for i, (word, label, token_start) in enumerate(
            zip(words, labels, list_token_start_map)
        ):
    ...
    def _apply_tokenizer(self, original_tokens, original_tags):
        ...
        for i, (original_token, original_tag) in enumerate(
            zip(original_tokens, original_tags)
        ):
...
class TransformersBasedTagger(LabelStudioMLBase):
    ...
    def load(self, train_output):
        ...
        self._pad_token = self._tokenizer.convert_tokens_to_ids(
    ...
    def predict(self, tasks, **kwargs):
        ...
        predict_set = SpanLabeledTextDataset(
            texts, tokenizer=self._tokenizer, **self._dataset_params_dict
        )
        ...
        for batch in tqdm(predict_loader, desc="Prediction"):
            ...
            for max_preds, argmax_preds, mask_tokens, token_start_map, string in zip(
                ...
                preds, scores, starts = [], [], []
                for max_pred, argmax_pred, mask_token, token_start in zip(
                    max_preds, argmax_preds, mask_tokens, token_start_map
                ):
                    if token_start != -1:
                ...
                for label, group in groupby(
                    zip(preds, starts, scores),
                    key=lambda i: re.sub("^(B-|I-)", "", i[0]),
                ):
                ...
                results.append(
                    {"result": result, "score": float(mean_score), "cluster": None}
                )
        return results
        ...
        for r in completion["result"]:
            if r["from_name"] == self.from_name and r["to_name"] == self.to_name:
                ...
                if not isinstance(labels, list) or len(labels) == 0:
                    logger.warning(
                        f'Error while parsing {r}: list type expected for "labels"'
                    )
                    continue
                label = labels[0]
                ...
                if start is None or end is None:
                    logger.warning(
                        f'Error while parsing {r}: "labels" should contain "start" and "end" fields'
                    )
    ...
    def fit(
        ...
        **kwargs,
        ...
        config = AutoConfig.from_pretrained(
            pretrained_model, num_labels=train_set.num_labels, cache_dir=cache_dir
        )
